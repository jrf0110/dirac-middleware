...
m.find = function(collection){
  return function(req, res){
    dirac.dals[ collection ].find(req.queryObj, req.queryOptions, function(error, results){
...
m.findOne = function(collection){
  return function(req, res){
    dirac.dals[ collection ].findOne(req.queryObj, req.queryOptions, function(error, result){
...
m.insert = function(collection){
  return function(req, res){
    ...
    dirac.dals[ collection ].insert(req.body, req.queryOptions, function(error, results){
      if (error) return console.log(error), res.error(400);
...
m.update = function(collection, options){
  ...
  return function(req, res){
    dirac.dals[ collection ].update(req.queryObj, req.body, req.queryOptions, function(error, results, result){
      if (error) return console.log(error), res.error(400);
...
m.remove = function(collection){
  return function(req, res){
    dirac.dals[ collection ].remove(req.queryObj, req.queryOptions, function(error, results, result){
      if (error) return res.error(400);
